...
def get_uno(mf, do_loc=True, iprint=True):
    ...
    vlmo = (
        scdm(v_orbs, ova, s12inv) if v_orbs.shape[1] != 0 else v_orbs
    )  # local "AOs" in external space
...
def select_active_space(mol, coeff, mo_occ, ao_labels, atom_order=None, iprint=1):
    ao_labels = [s.replace(" ", "-") for s in ao_labels]
    ...
    for iorb in range(mol.nao):
        ...
        for iao in ivs[::-1][:3]:
            ...
            text += ltexts[-1]
        if any(x in ltexts[0] for x in ao_labels):
            if iprint:
                print(" *", text)
